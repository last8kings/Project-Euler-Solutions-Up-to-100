...
int main() {
    ...
    for (int i = 0; i < 20; ++i) {
        for (int j = 0; j < 20; ++j) {
            int down = 1, right = 1, diagr = 1, diagl = 1;  // only need these three because if a given number's up product
                                                // can be thought of as the down product of the space 4 above it
                                                // Same logic applies to right and the diagonals.
            if (i < 17) {
                for (int k = 0; k < 4; ++k) {
                    down *= grid[i + k][j];
                }
